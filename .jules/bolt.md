...
## 2025-05-21 - Pre-caching config set lookups and fast-path string checks before regex parsing in QA checkers
**Learning:** Instantiating temporary `set` objects from configuration lists (`set(list)`) inside hot item-evaluation loops creates hundreds of thousands of redundant set allocations and garbage-collection overhead. Precomputing sets in configuration initialization turns dynamic set construction into O(1) lookup. Furthermore, adding a fast-path character check (such as `'<' in str`) before invoking regex parsers for markup tag analysis avoids expensive regex engine setup and matching across plain-text strings, yielding >2.1x speedup in translation catalog validation.
**Action:** Pre-build immutable lookup sets on config objects during initialization. Always use simple substring character checks as fast-path guards before executing regex searches in hot processing loops.
