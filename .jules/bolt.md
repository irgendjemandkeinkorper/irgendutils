...
## 2025-05-21 - Single-pass character scanning and compiled rule caching for HTML/content analysis
**Learning:** In content analysis workflows processing thousands of HTML pages, using string regex replacements (`.replace(/<[^>]*>/g, ' ').split(/\s+/)`) creates severe garbage collection pressure from intermediate string copies and array allocations. Replacing this with a fast single-pass character scanning loop (`charCodeAt`) yields a ~7.4x speedup in word count calculation. Additionally, caching compiled `RegExp` instances on config rule objects and reusing resolved rule objects across analysis phases prevents redundant regular expression compilation and path resolution per page.
**Action:** Use single-pass character scanning loops for HTML word counting to avoid regex/array allocations, and store pre-resolved rule configurations on item context objects during batch processing.
