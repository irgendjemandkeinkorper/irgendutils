...
## 2025-05-21 - Combining sequential string replacements and caching RegExp compilation in HTML tag extraction
**Learning:** Executing multiple back-to-back `.replace()` calls with single-target regexes on full HTML documents forces V8 to perform multiple string scans and intermediate string buffer allocations. Combining script, style, tag, and entity stripping patterns into a single global OR regex (`/<script\b[\s\S]*?<\/script>|<style\b[\s\S]*?<\/style>|<[^>]+>|&nbsp;/gi`) allows a single pass replacement. Furthermore, caching tag/block regexes and pre-compiling module-level `RegExp` instances avoids allocating new `RegExp` instances on every function invocation.
**Action:** Combine sequential string replacement steps on large string documents into a single global disjunction regex pattern, and hoist static or cached RegExp object creation to module scope.
