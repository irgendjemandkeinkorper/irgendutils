...
## 2025-05-21 - Fast string-slicing relative path computation and pre-normalized exclusion checks in file tree scanners
**Learning:** In recursive directory scanners (`os.scandir` loops), calling `os.path.relpath` on every file entry incurs massive performance overhead due to repeated path splitting, normalization, and component comparisons across thousands of iterations. Furthermore, repeatedly calling `os.path.abspath` on exclusion patterns inside file-scanning loops introduces redundant $O(N \times M)$ path resolution overhead. Replacing `os.path.relpath` with a string prefix-slicing helper (`fast_relpath`) and pre-normalizing exclusion paths once before traversal reduces scan runtime by ~46% (~1.85x speedup).
**Action:** Use fast string slicing for child path relativity when `path.startswith(root_prefix)` and pre-normalize exclusion paths outside traversal loops.
