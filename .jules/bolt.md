...
## 2025-05-21 - charCodeAt scanning and direct pointer tracking in hot parser loops
**Learning:** In hot sequential parsing loops, evaluating simple character classes (such as HTML tag whitespace delimiters) via regex engine methods like `/\s/.test()` incurs significant setup and execution overhead per character. Replacing regex checks with direct `charCodeAt` ASCII character code comparisons (`32`, `9`, `10`, `13`, `12`) speeds up character scanning. Furthermore, tracking stack top elements directly in a reference variable rather than calling closure functions (e.g. `top()`) eliminates repeated call stack frame creations.
**Action:** Use direct `charCodeAt` code comparisons for basic whitespace or delimiter scanning inside hot parsing loops, and maintain explicit pointer/reference variables for top stack items.
