...
## 2025-09-09 - Pre-converting policy lists to sets and module-scoping regexes in evaluation engines
**Learning:** Checking item membership against policy configuration lists (like allowed/disallowed lists) inside tight iteration loops (e.g., evaluating hundreds of plugins/themes against policy rules) incurs an $O(N)$ linear scanning bottleneck per item. Pre-converting policy lists to `set`s on engine initialization converts lookups into $O(1)$ operations (~30x faster for large lists). Additionally, pre-compiling regular expressions (like `re.compile(r'\d+')`) at module scope prevents repeated regex compilation overhead on every function call.
**Action:** Pre-convert list configurations to sets during class initialization when evaluating items in loops, and hoist static regex patterns to module scope.
