...
## 2025-05-21 - Replacing linear queue scans and array pops with deque and set lookups in crawler state queues
**Learning:** Using a standard Python list for crawler task queues causes two $O(N)$ bottlenecks: linear array scans (`any(item['url'] == url for item in self.queue)`) during duplicate checking and front pops (`pop(0)` / `insert(0)`). Parallelizing set tracking via a `queued_urls` set turns duplicate checks into $O(1)$ set lookups, and replacing the list with `collections.deque` turns front queue operations into $O(1)$ calls (`popleft()` and `appendleft()`). This yields a >70x speedup for large batch URL processing while preserving exact queue ordering and JSON serialization logic.
**Action:** Always maintain a `set` alongside `collections.deque` for work queues requiring membership checks or front pops/inserts. Convert `deque` back to `list` only during state serialization.
