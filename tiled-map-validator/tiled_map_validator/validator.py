...
class TiledValidator:
    ...
    def validate_map(self, map_path: str) -> List[Dict[str, Any]]:
        ...
        # BOLT OPTIMIZATION: Extract (firstgid, lastgid) ranges and memoize GID validity checks
        # across tile layer data loops and object GID lookups using `gid_cache`.
        # This reduces repetitive list iterations over tilesets for millions of map tiles,
        # speeding up map validation by ~50%.
        tileset_ranges = [(ts["firstgid"], ts["lastgid"]) for ts in resolved_tilesets]
        gid_cache: Dict[int, bool] = {}

        def check_valid_gid(clean_gid: int) -> bool:
            valid = gid_cache.get(clean_gid)
            if valid is None:
                valid = any(low <= clean_gid <= high for low, high in tileset_ranges)
                    ...
                    if not is_infinite:
                        ...
                        if not isinstance(gids, list):
                            ...
                            for idx, raw_gid in enumerate(gids):
                                ...
                                if clean_gid == 0:
                                    continue
                                # Validate GID is covered by tilesets using memoized lookup
                                if not check_valid_gid(clean_gid):
                        ...
                        if not isinstance(chunks, list):
                            ...
                            for c_idx, chunk in enumerate(chunks):
                                ...
                                for idx, raw_gid in enumerate(c_data):
                                    ...
                                    if clean_gid == 0:
                                        continue
                                    # Validate GID using memoized lookup
                                    if not check_valid_gid(clean_gid):
                    ...
                    if not isinstance(objects, list):
                        ...
                        for obj in objects:
                            ...
                            if obj_gid_raw is not None:
                                ...
                                if obj_gid > 0:
                                    if not check_valid_gid(obj_gid):
                                        self._add_finding(
                                            findings, map_path, "error", "gid",
