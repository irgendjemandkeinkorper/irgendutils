...
// BOLT OPTIMIZATION: Module-scoped regular expressions and caches to avoid
// repeated RegExp instantiation and compilation on every HTML extraction/tag parsing call.
const ATTR_RE = /([a-zA-Z][\w:-]*)(?:\s*=\s*(?:"([^"]*)"|'([^']*)'|([^\s"'>]+)))?/g;

const TAG_RE_CACHE = new Map();
function getTagRe(name) {
...
const FG_COLOR_RE = /(?:^|;)\s*color\s*:\s*(#[0-9a-fA-F]{3,8})/;
const BG_COLOR_RE = /background(?:-color)?\s*:\s*(#[0-9a-fA-F]{3,8})/;
const REL_SPLIT_RE = /\s+/;
const FAVICON_REL_RE = /(^|\s)(icon|shortcut|apple-touch-icon|mask-icon)(\s|$)/;
...
export function extract(html) {
  ...
  const canonical = linkTags.find((a) => relOf(a).split(REL_SPLIT_RE).includes('canonical'))?.href ?? null;
  const hasFaviconLink = linkTags.some((a) => FAVICON_REL_RE.test(relOf(a)));
  const stylesheets = linkTags.filter((a) => relOf(a).split(REL_SPLIT_RE).includes('stylesheet')).map((a) => a.href || '');
  ...
  while ((m = STYLE_BLOCK_RE.exec(h))) styleBlocks.push(m[1]);
  ...
  while ((m = INLINE_STYLE_RE.exec(h))) {
    const style = m[1] ?? m[2];
    const fg = style.match(FG_COLOR_RE);
    const bg = style.match(BG_COLOR_RE);
